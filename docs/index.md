...
| *cloud credentials* | No* | Azure: `azure_tenant_id`, `azure_subscription_id`, `azure_client_id`. AWS: `aws_region`, `aws_role_arn`, `aws_registry_secret_arn`. Optional per run — each falls back to its `PROVISION_*` repository variable (the organization default); an input always overrides the variable, and with neither the run fails fast naming both |
